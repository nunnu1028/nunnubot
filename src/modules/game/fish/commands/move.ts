...
export class FishMoveCommand implements Command {
	...
	public async execute(info: MessageInfo): Promise<void> {
		...
		if (!roomName) return info.replier.reply("방 이름을 입력해주세요!");

		const roomInfo = FishUtils.FISH_DATABASE.lastData.rooms.find((e) => e.name === roomName);
		if (!roomInfo) return info.replier.reply("방을 찾을 수 없어요! 다시 확인해줄래요?");
		if (user.currentRoomId === roomInfo.id) return info.replier.reply("이미 그 방에 있어요!");
		if (user.currentLevel < roomInfo.requiredLevel) return info.replier.reply("그 방에 들어가기에는 레벨이 부족해요!");

		user.currentRoomId = roomInfo.id;
		FishUtils.FISH_DATABASE.save(FishUtils.FISH_DATABASE.lastData);

		info.replier.reply(`방 이동이 완료되었어요!\n현재 계신 방 이름은 ${roomInfo.name} 이에요!`);
	}
}
